...
class MBUSASpider(scrapy.Spider):
    name = 'MBUSA'
    url = 'https://nafta-service.mbusa.com/api/inv/en_us/used/vehicles/search?'
    detail_url = 'https://nafta-service.mbusa.com/api/inv/en_us/used/vehicles/lookup/vin?vin='
    page_num = 0
    page_count = 100
    default_model = [
    ...
    # bodyStyle=CAB:RDS,SDN:HBK,CPE,SUV:WGN
    ...
    def __init__(self, year=None, model=None, body=None, detail=None, *args, **kwargs):
        ...
        super(MBUSASpider, self).__init__(*args, **kwargs)
    ...
    def parse_item(self, response):
        ...
        if len(vehicles) > 0:
            ...
            content_url = 'https://www.mbusa.com/en/cpo/inventory/vehicle/'+str(dealerCode)+'/'+str(dealerId)+'/'+str(vehicles.get('classId'))+'/'+str(vehicles.get('bodystyleId'))+'/'+str(vehicles.get('modelId'))+'/'+str(vehicles.get('vin'))
            features = pluck(vehicle_data.get('optionList'), 'text') + pluck(vehicle_data.get('comfortList'), 'text') + pluck(vehicle_data.get('safetyList'), 'text') + pluck(vehicle_data.get('performanceList'), 'text')
            ...
            item['price']           = vehicle_data.get('dsrp')
            item['mileage']         = vehicle_data.get('mileage')
            ...
            item['cylinders']       = CarpostFilter.formatCyl(CarpostFilter.getCyl(pluck(vehicle_data.get('performanceList'), 'text')))
            ...
            item['engine']          = CarpostFilter.formatEngine(CarpostFilter.getEnge(pluck(vehicle_data.get('performanceList'), 'text')))
